...
module ineqsudx_scan_solver #(
    ...
    typedef enum logic [3:0] {
        IDLE, INIT, SCAN, DOMAIN, PICK_ROWS, PICK_CELL, PLACE,
    ...
    endgenerate
    // Count candidates during the existing SCAN cycle. MRV is entered only
    // after DOMAIN reports no changes, so these counts still describe the
    // domains selected by PICK_ROWS. This cuts the count-to-tournament path
    // without adding a cycle or changing the MRV tie order.
    logic [3:0] cell_count_q [0:80];
    generate
        for (c=0; c<81; c++) begin : CANDIDATE_COUNTS
        ...
        for (r = 0; r < 9; r++) begin : ROW_MRV
            for (k = 0; k < 16; k++) begin : LEAF
                if (k < 9) begin : REAL_CELL
                    assign row_count[r][16+k] = cell_count_q[r*9+k];
