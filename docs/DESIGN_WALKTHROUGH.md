...
The final version is **v5**, with its RTL built from commit **f5a9e9a**. Later
...
We have not proved that v5 is the fastest possible design.**
...
Possible later work:
    Target the measured critical path.
    Reduce guess overhead if timing permits.
...
explains the next opportunities and their tradeoffs.
...
Standalone accelerator Fmax         = 58.10 MHz
Full-system reported Fmax           = 52.11 MHz
Configured physical hardware clock  = 50 MHz

Assignment score = 483 / 58.10 = 8.31 microseconds
...
AlwaysSudx v5:
    Mean core cycles = 387.5459
    Core time estimate = 387.5459 / 58.10 = 6.67 us

Ratio = 21.31 / 6.67 = 3.20x
```
...
but v5's higher measured frequency gives a lower core-time estimate.
...
releases. Only v5 has a complete bitstream from this work. A v5 bitstream must never
be labeled as v0/v1/v2/v3/v4 hardware.
...
**Is 58.10 MHz the physical clock?** No. It is standalone Fmax used by the assignment
score. The bitstream runs at the course's default 50 MHz.
...
## 20. What could improve next, and why it was not changed in this release
...
logs/v5/build_source.json               RTL and bitstream SHA-256 hashes
logs/v5/synthesis/                      Standalone reports
logs/v5/fpga/                           Complete-system reports
```
...
programming files with the new source. Keep the original v5 release as a reproducible
reference while exploring the next hypothesis.
