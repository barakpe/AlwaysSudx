...
for SUDX_EXT in sof svf; do
    if [ -e "$SUDX_DROP/k5_xbox_ineqsudx_scan.$SUDX_EXT" ]; then
        echo "Refusing to overwrite an existing course artifact: $SUDX_DROP/k5_xbox_ineqsudx_scan.$SUDX_EXT" >&2
        exit 1
    fi
done
comp_fpga ineqsudx_scan
for SUDX_EXT in sof svf; do
    mv "$SUDX_DROP/k5_xbox_ineqsudx_scan.$SUDX_EXT" "prog_files/k5_xbox_ineqsudx_scan.$SUDX_EXT"
done
test -s prog_files/k5_xbox_ineqsudx_scan.sof
