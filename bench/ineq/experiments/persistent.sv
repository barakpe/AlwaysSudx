...
module ineqsudx_scan_solver #(
    ...
    logic [15:0] top;
    // Wide synchronous RAM stores the parent's domains before each guess.
    // Only guesses consume entries; retries restore the same parent snapshot.
    logic [728:0] domain_stack [0:80];
    logic [728:0] saved_domains;
    always_ff @(posedge clk) begin
        if (rst_n) begin
            if (state == PLACE) domain_stack[depth] <= candidate;
            if (state == BACK_READ && depth != 0)
                saved_domains <= domain_stack[depth-7'd1];
        end
    end
                ...
                SCAN: begin
                    for (int c = 0; c < 81; c++) begin
                        candidate[c] <= value[c] != 0 ? 9'd0 :
                            candidate[c] & ~(occupied[c/9] | occupied[9+c%9] | occupied[18+(c/27)*3+(c%9)/3]);
                        empty_q[c] <= value[c] == 0;
                    end
                    used_q <= occupied;
